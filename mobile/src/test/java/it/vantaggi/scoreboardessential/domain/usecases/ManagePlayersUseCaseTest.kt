...
class ManagePlayersUseCaseTest {
    ...
    @Test
    fun `addPlayerToTeam adds player to team 1`() = runTest {
        // Given
        ...
        // Then
        assertEquals(1, useCase.teamRoster.first().team1Players.size)
        assertEquals(0, useCase.teamRoster.first().team2Players.size)
        verify(wearDataSync).syncTeamPlayers(any(), any())
    }

    @Test
    fun `addPlayerToTeam adds player to team 2`() = runTest {
        // Given
        val player = PlayerWithRoles(Player(1, "Player 1", 0, 0), emptyList())
        ...
        // Then
        assertEquals(0, useCase.teamRoster.first().team1Players.size)
        assertEquals(1, useCase.teamRoster.first().team2Players.size)
        verify(wearDataSync).syncTeamPlayers(any(), any())
    }

    @Test
    fun `addPlayerToTeam does not add duplicate player`() = runTest {
        // Given
        val player = PlayerWithRoles(Player(1, "Player 1", 0, 0), emptyList())
        useCase.addPlayerToTeam(player, 1)
    ...
    @Test
    fun `removePlayerFromTeam removes player from team 1`() = runTest {
        // Given
        val player = PlayerWithRoles(Player(1, "Player 1", 0, 0), emptyList())
        useCase.addPlayerToTeam(player, 1)
    ...
    @Test
    fun `removePlayerFromTeam removes player from team 2`() = runTest {
        // Given
        val player = PlayerWithRoles(Player(1, "Player 1", 0, 0), emptyList())
        useCase.addPlayerToTeam(player, 2)
