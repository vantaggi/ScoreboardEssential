...
fun addPlayerToTeam(player: PlayerWithRoles, teamNumber: Int) {
val currentRoster = _teamRoster.value

val newRoster = when (teamNumber) {
1 -> {
...
fun removePlayerFromTeam(player: PlayerWithRoles, teamNumber: Int) {
val currentRoster = _teamRoster.value

val newRoster = when (teamNumber) {
1 -> {
val updatedTeam1 = currentRoster.team1Players
.filter { it.player.playerId != player.player.playerId }
currentRoster.copy(team1Players = updatedTeam1)
}
2 -> {
val updatedTeam2 = currentRoster.team2Players
.filter { it.player.playerId != player.player.playerId }
currentRoster.copy(team2Players = updatedTeam2)
}
else -> return
